...
metal = ['La','Ce','Pr','Nd','Sm','Eu','Gd','Lu']
##peptideFlag polypeptide = ['LBT3-', 'LBT5-']
replicate = [0] # , 1, 2]
##peptideFlag lambda_LJ   = [1.0] # [0.0, 0.1, 0.25, 0.4, 0.55, 0.7, 0.85, 1.0] 
##peptideFlag lambda_ELE  = [1.0] # [0.0, 0.075, 0.150, 0.225, 0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0]
    ...
    for j in range(len(replicate)):
        for k in range(len(lambda_ELE)):
            if lambda_ELE[k] == lambda_ELE[-1]:
                ##peptideFlag for m in range(len(polypeptide)):
                    for l in range(len(lambda_LJ)):
                        statepoint = {
                            ...
                            ##peptideFlag "polypeptide": polypeptide[m]
                        }
